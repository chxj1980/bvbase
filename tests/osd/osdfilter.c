...
static int init_filter(AVFilterGraph *filter_graph, BVOSDConfig *config)
{
	...
	snprintf(args, sizeof(args),"video_size=%dx%d:pix_fmt=%d:time_base=1/%d", config->width, config->height, config->src_fmt, 25);
...
int ff_osd_update_data(AVFilterGraph *filter_graph, AVPacket *packet)
{
    ...
    if (av_opt_set(osd_drawtext->priv, "text", (char *)packet->data, 0) < 0) {
...
int ff_osd_process(AVFilterGraph *filter_graph, AVFrame *src_frm, AVFrame *dst_frm)
{
    ...
    if (av_buffersrc_add_frame_flags(buffersrc_ctx, src_frm, AV_BUFFERSRC_FLAG_KEEP_REF) < 0) {
