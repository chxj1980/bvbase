...
static int onvif_probe(BVProbeData *p)
{
...
static int set_avcodec_info(BVMediaContext *s, OnvifContext *onvifctx)
{
	...
	for (i = 0; i < onvif->nb_streams; ++i) {
		in_stream = onvif->streams[i];
		out_stream = avformat_new_stream(s, in_stream->codec->codec);
		if (!out_stream)
		...
		if (in_stream->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
			set_video_info(out_stream, onvifctx);
		}
	}
	return 0;
}
...
static int bv_onvif_stream_uri(OnvifContext *onvifctx)
{
	...
	if (retval != SOAP_OK) {
		...
		strncpy(onvifctx->onvif_url, trt__GetStreamUriResponse.MediaUri->Uri, sizeof(onvifctx->onvif_url));
		ret = 0;
	}
	return ret;
}
...
static bv_cold int onvif_read_header(BVMediaContext * s)
{
    ...
    onvifctx->soap = bv_soap_new(onvifctx->timeout);
    if (!onvifctx->soap)
...
{
    pkt->data = bv_memdup(rpkt->data, rpkt->size);
    if (!pkt->data) {
