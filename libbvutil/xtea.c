...
void bv_xtea_init(BVXTEA *ctx, const uint8_t key[16])
{
    int i;

    for (i = 0; i < 4; i++)
        ctx->key[i] = BV_RB32(key + (i << 2));
}

static void xtea_crypt_ecb(BVXTEA *ctx, uint8_t *dst, const uint8_t *src,
...
void bv_xtea_crypt(BVXTEA *ctx, uint8_t *dst, const uint8_t *src, int count,
...
static void test_xtea(BVXTEA *ctx, uint8_t *dst, const uint8_t *src,
                      const uint8_t *ref, int len, uint8_t *iv, int dir,
                      const char *test)
{
    bv_xtea_crypt(ctx, dst, src, len, iv, dir);
...
int main(void)
{
    ...
    for (i = 0; i < XTEA_NUM_TESTS; i++) {
        bv_xtea_init(&ctx, xtea_test_key[i]);
        ...
        bv_xtea_crypt(&ctx, ct, src, 4, iv, 0);
