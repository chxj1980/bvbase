...
static const uint32_t orig_p[BV_BF_ROUNDS + 2] = {
...
bv_cold void bv_blowfish_init(BVBlowfish *ctx, const uint8_t *key, int key_len)
{
    ...
    for (i = 0; i < BV_BF_ROUNDS + 2; i += 2) {
        bv_blowfish_crypt_ecb(ctx, &data_l, &data_r, 0);
        ctx->p[i]     = data_l;
        ctx->p[i + 1] = data_r;
    }
...
void bv_blowfish_crypt_ecb(BVBlowfish *ctx, uint32_t *xl, uint32_t *xr,
                           int decrypt)
{
    uint32_t Xl, Xr;
    int i;
    ...
    if (decrypt) {
        Xl ^= ctx->p[BV_BF_ROUNDS + 1];
        for (i = BV_BF_ROUNDS; i > 0; i-=2) {
            F(Xl, Xr, ctx->p[i  ]);
            F(Xr, Xl, ctx->p[i-1]);
        }
        ...
        Xl ^= ctx->p[0];
        for (i = 1; i < BV_BF_ROUNDS+1; i+=2){
            F(Xl, Xr, ctx->p[i  ]);
            F(Xr, Xl, ctx->p[i+1]);
        }

        Xr ^= ctx->p[BV_BF_ROUNDS + 1];
    }
...
void bv_blowfish_crypt(BVBlowfish *ctx, uint8_t *dst, const uint8_t *src,
                       int count, uint8_t *iv, int decrypt)
{
    ...
    if (decrypt) {
        while (count--) {
            ...
            bv_blowfish_crypt_ecb(ctx, &v0, &v1, decrypt);
...
static void test_blowfish(BVBlowfish *ctx, uint8_t *dst, const uint8_t *src,
                          const uint8_t *ref, int len, uint8_t *iv, int dir,
                          const char *test)
{
    bv_blowfish_crypt(ctx, dst, src, len, iv, dir);
...
int main(void)
{
    ...
    bv_blowfish_init(&ctx, "abcdefghijklmnopqrstuvwxyz", 26);
    ...
    for (i = 0; i < NUM_VARIABLE_KEY_TESTS; i++) {
        bv_blowfish_init(&ctx, variable_key[i], 8);

        bv_blowfish_crypt_ecb(&ctx, &tmptext_l[i], &tmptext_r[i], 0);
        if (tmptext_l[i] != ciphertext_l[i] || tmptext_r[i] != ciphertext_r[i]) {
            printf("Test encryption failed.\n");
            return 1;
        }

        bv_blowfish_crypt_ecb(&ctx, &tmptext_l[i], &tmptext_r[i], 1);
