...
void bv_bprint_init(BVBPrint *buf, unsigned size_init, unsigned size_max)
{
    ...
    *buf->str = 0;
    if (size_init > buf->size)
        bv_bprint_alloc(buf, size_init - 1);
}
...
void bv_bprintf(BVBPrint *buf, const char *fmt, ...)
{
    unsigned room;
    char *dst;
    va_list vl;
    ...
    while (1) {
        ...
        if (extra_len < room)
            break;
        if (bv_bprint_alloc(buf, extra_len))
            break;
    }
    bv_bprint_grow(buf, extra_len);
}
...
void bv_bprint_strftime(BVBPrint *buf, const char *fmt, const struct tm *tm)
{
    ...
    bv_bprint_grow(buf, l);
}

void bv_bprint_get_buffer(BVBPrint *buf, unsigned size,
                          unsigned char **mem, unsigned *actual_size)
{
    if (size > bv_bprint_room(buf))
        bv_bprint_alloc(buf, size);
    *actual_size = bv_bprint_room(buf);
...
int bv_bprint_finalize(BVBPrint *buf, char **ret_str)
{
    ...
    if (ret_str) {
        if (bv_bprint_is_allocated(buf)) {
            str = bv_realloc(buf->str, real_size);
            if (!str)
                str = buf->str;
            buf->str = NULL;
        } else {
            str = bv_malloc(real_size);
            if (str)
                      ...
                      enum BVEscapeMode mode, int flags)
{
    ...
    if (mode == BV_ESCAPE_MODE_AUTO)
        mode = BV_ESCAPE_MODE_BACKSLASH; /* TODO: implement a heuristic */

    switch (mode) {
    case BV_ESCAPE_MODE_QUOTE:
        /* enclose the string between '' */
        bv_bprint_chars(dstbuf, '\'', 1);
        for (; *src; src++) {
            if (*src == '\'')
                bv_bprintf(dstbuf, "'\\''");
            else
                bv_bprint_chars(dstbuf, *src, 1);
        }
        bv_bprint_chars(dstbuf, '\'', 1);
        break;

    /* case BV_ESCAPE_MODE_BACKSLASH or unknown mode */
    default:
        /* \-escape characters */
        for (; *src; src++) {
            ...
            int is_special          =
                ...
                (is_ws && (flags & BV_ESCAPE_FLAG_WHITESPACE));

            if (is_strictly_special ||
                (!(flags & BV_ESCAPE_FLAG_STRICT) &&
...
static void bprint_pascal(BVBPrint *b, unsigned size)
{
    unsigned i, j;
    unsigned p[42];
...
int main(void)
{
    ...
    bv_bprint_init(&b, 0, 0);
    bprint_pascal(&b, 25);
    printf("Long text count only buffer: %u/%u\n", (unsigned)strlen(b.str), b.len);

    bv_bprint_init_for_buffer(&b, buf, sizeof(buf));
    bprint_pascal(&b, 25);
    printf("Long text count only buffer: %u/%u\n", (unsigned)strlen(buf), b.len);

    bv_bprint_init(&b, 0, -1);
    bv_bprint_strftime(&b, "%Y-%m-%d", &testtime);
    printf("strftime full: %u/%u \"%s\"\n", (unsigned)strlen(buf), b.len, b.str);
    bv_bprint_finalize(&b, NULL);

    bv_bprint_init(&b, 0, 8);
    bv_bprint_strftime(&b, "%Y-%m-%d", &testtime);
