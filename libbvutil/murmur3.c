...
int main(void)
{
    ...
    {
        in[i] = i;
        bv_murmur3_init_seeded(ctx, 256 - i);
        // Note: this actually tests hashing 0 bytes
        bv_murmur3_update(ctx, in, i);
        bv_murmur3_final(ctx, hashes + 16 * i);
    }
    bv_murmur3_init_seeded(ctx, 0);
    bv_murmur3_update(ctx, hashes, 256 * 16);
    bv_murmur3_final(ctx, hash_result);
    bv_free(hashes);
    bv_freep(&ctx);
    printf("result: 0x%"PRIx64" 0x%"PRIx64"\n", BV_RL64(hash_result), BV_RL64(hash_result + 8));
    // official reference value is 32 bit
    return BV_RL32(hash_result) != 0x6384ba69;
#else
    uint8_t *in = bv_mallocz(512*1024);
    bv_murmur3_init(ctx);
    for (i = 0; i < 40*1024; i++)
        bv_murmur3_update(ctx, in, 512*1024);
    bv_murmur3_final(ctx, hash_result);
    bv_free(in);
