...
static inline void copy_backptr(LZOContext *c, int back, int cnt)
{
    ...
    if (dst - c->out_start < back) {
        c->error |= BV_LZO_INVALID_BACKPTR;
        return;
    }
    if (cnt > c->out_end - dst) {
        cnt       = BBMAX(c->out_end - dst, 0);
...
int bv_lzo1x_decode(void *out, int *outlen, const void *in, int *inlen)
{
    int state = 0;
    int x;
    LZOContext c;
    if (*outlen <= 0 || *inlen <= 0) {
        int res = 0;
        if (*outlen <= 0)
            res |= BV_LZO_OUTPUT_FULL;
        if (*inlen <= 0)
            res |= BV_LZO_INPUT_DEPLETED;
        return res;
    }
    c.in      = in;
...
int main(int argc, char *argv[]) {
    ...
    uint8_t *orig = bv_malloc(MAXSZ + 16);
    uint8_t *comp = bv_malloc(2*MAXSZ + 16);
    uint8_t *decomp = bv_malloc(MAXSZ + 16);
...
#else
        if (bv_lzo1x_decode(decomp, &outlen, comp, &inlen))
#endif
            bv_log(NULL, BV_LOG_ERROR, "decompression error\n");
STOP_TIMER("lzod")
    }
    if (memcmp(orig, decomp, s))
        bv_log(NULL, BV_LOG_ERROR, "decompression incorrect\n");
    else
        bv_log(NULL, BV_LOG_ERROR, "decompression OK\n");
    fclose(in);
    return 0;
}
#endif
