...
static bv_always_inline int sad_wxh(const uint8_t *src1, ptrdiff_t stride1,
...
{
    int out, ref;
    bv_pixelutils_sad_fn f_ref = sad_c[n - 1];
    bv_pixelutils_sad_fn f_out = bv_pixelutils_get_sad_fn(n, n, align, NULL);
...
{
    int i, align, ret;
    uint8_t *buf1 = bv_malloc(W1*H1);
    uint8_t *buf2 = bv_malloc(W2*H2);
    uint32_t state = 0;
...
#define RANDOM_INIT(buf, size) do {             \
    ...
    for (i = 1; i <= 4; i++) {
        for (align = 0; align < 3; align++) {
            ...
            buf1 = bv_malloc(size1);
            buf2 = bv_malloc(size2);
            if (!buf1 || !buf2) {
                fprintf(stderr, "malloc failure\n");
                ret = 1;
                goto end;
