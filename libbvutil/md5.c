...
void bv_md5_final(AVMD5 *ctx, uint8_t *dst)
{
    ...
    bv_md5_update(ctx, "\200", 1);
    while ((ctx->len & 63) != 56)
        bv_md5_update(ctx, "", 1);

    bv_md5_update(ctx, (uint8_t *)&finalcount, 8);

    for (i = 0; i < 4; i++)
        BV_WL32(dst + 4*i, ctx->ABCD[3 - i]);
}
...
int main(void){
    ...
    for (i = 0; i < 1000; i++)
        in[i] = i * i;
    bv_md5_sum(md5val, in, 1000); print_md5(md5val);
    bv_md5_sum(md5val, in,   63); print_md5(md5val);
    bv_md5_sum(md5val, in,   64); print_md5(md5val);
    bv_md5_sum(md5val, in,   65); print_md5(md5val);
    for (i = 0; i < 1000; i++)
        in[i] = i % 127;
    bv_md5_sum(md5val, in,  999); print_md5(md5val);
