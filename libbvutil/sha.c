...
const int bv_sha_size = sizeof(BVSHA);

struct BVSHA *bv_sha_alloc(void)
{
    return bv_mallocz(sizeof(struct BVSHA));
}

#define rol(value, bits) (((value) << (bits)) | ((value) >> (32 - (bits))))

/* (R0+R1), R2, R3, R4 are the different operations used in SHA1 */
#define blk0(i) (block[i] = BV_RB32(buffer + 4 * (i)))
...
bv_cold int bv_sha_init(BVSHA *ctx, int bits)
{
...
void bv_sha_update(BVSHA* ctx, const uint8_t* data, unsigned int len)
{
    unsigned int i, j;
...
void bv_sha_final(BVSHA* ctx, uint8_t *digest)
{
    int i;
    uint64_t finalcount = bv_be2ne64(ctx->count << 3);

    bv_sha_update(ctx, "\200", 1);
    while ((ctx->count & 63) != 56)
        bv_sha_update(ctx, "", 1);
    bv_sha_update(ctx, (uint8_t *)&finalcount, 8); /* Should cause a transform() */
    for (i = 0; i < ctx->digest_len; i++)
        BV_WB32(digest + i*4, ctx->state[i]);
}
...
int main(void)
{
    ...
    for (j = 0; j < 3; j++) {
        ...
        for (k = 0; k < 3; k++) {
            bv_sha_init(&ctx, lengths[j]);
            if (k == 0)
                bv_sha_update(&ctx, "abc", 3);
            else if (k == 1)
                bv_sha_update(&ctx, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", 56);
            else
                for (i = 0; i < 1000*1000; i++)
                    bv_sha_update(&ctx, "a", 1);
            bv_sha_final(&ctx, digest);
