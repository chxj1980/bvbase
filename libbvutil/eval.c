...
static const BVClass eval_class = { "Eval", bv_default_item_name, NULL, LIBBVUTIL_VERSION_INT, offsetof(Parser,log_offset), offsetof(Parser,log_ctx) };
...
static double eval_expr(Parser *p, AVExpr *e)
{
    switch (e->type) {
        ...
        case e_ld:     return e->value * p->var[bv_clip(eval_expr(p, e->param[0]), 0, VARS-1)];
        ...
        case e_clip: {
            ...
            if (isnan(min) || isnan(max) || isnan(x) || min > max)
                return NAN;
            return e->value * bv_clipd(eval_expr(p, e->param[0]), min, max);
        }
        case e_between: {
        ...
        case e_print: {
            double x = eval_expr(p, e->param[0]);
            int level = e->param[1] ? bv_clip(eval_expr(p, e->param[1]), INT_MIN, INT_MAX) : BV_LOG_INFO;
            bv_log(p, level, "%f\n", x);
            return x;
        }
        case e_random:{
            int idx= bv_clip(eval_expr(p, e->param[0]), 0, VARS-1);
        ...
        case e_taylor: {
            ...
            int id = e->param[2] ? bv_clip(eval_expr(p, e->param[2]), 0, VARS-1) : 0;
            int i;
        ...
        case e_root: {
            ...
            for(i=-1; i<1024; i++) {
                if(i<255) {
                    p->var[0] = bv_reverse[i&255]*x_max/255;
                } else {
        ...
        default: {
            ...
            switch (e->type) {
                case e_mod: return e->value * (d - floor((!BV_CONFIG_FTRAPV || d2) ? d / d2 : d * INFINITY) * d2);
                case e_gcd: return e->value * bv_gcd(d,d2);
                ...
                case e_div: return e->value * ((!BV_CONFIG_FTRAPV || d2 ) ? (d / d2) : d * INFINITY);
                case e_add: return e->value * (d + d2);
                case e_last:return e->value * d2;
                case e_st : return e->value * (p->var[bv_clip(d, 0, VARS-1)]= d2);
...
void bv_expr_free(AVExpr *e)
{
    ...
    bv_expr_free(e->param[1]);
    bv_expr_free(e->param[2]);
    bv_freep(&e->var);
    bv_freep(&e);
}
...
static int parse_primary(AVExpr **e, Parser *p)
{
    ...
    p->s= strchr(p->s, '(');
    if (!p->s) {
        bv_log(p, BV_LOG_ERROR, "Undefined constant or missing '(' in '%s'\n", s0);
        p->s= next;
        bv_expr_free(d);
    ...
    if (*next == '(') { // special case do-nothing
        ...
        if ((ret = parse_expr(&d, p)) < 0)
            return ret;
        if (p->s[0] != ')') {
            bv_log(p, BV_LOG_ERROR, "Missing ')' in '%s'\n", s0);
            bv_expr_free(d);
            return BVERROR(EINVAL);
        }
        p->s++; // ")"
    ...
    if (p->s[0] != ')') {
        bv_log(p, BV_LOG_ERROR, "Missing ')' or too many args in '%s'\n", s0);
        bv_expr_free(d);
        return BVERROR(EINVAL);
    }
    p->s++; // ")"
    ...
    d->type = e_func0;
        ...
        bv_log(p, BV_LOG_ERROR, "Unknown function in '%s'\n", s0);
        bv_expr_free(d);
        return BVERROR(EINVAL);
    }
...
static int parse_dB(AVExpr **e, Parser *p, int *sign)
{
    ...
    if (*p->s == '-') {
        char *next;
        double bv_unused ignored = strtod(p->s, &next);
                  ...
                  int log_offset, void *log_ctx)
{
    ...
    *wp++ = 0;
    ...
    if (*p.s) {
        bv_expr_free(e);
        bv_log(&p, BV_LOG_ERROR, "Invalid chars '%s' at the end of expression '%s'\n", p.s, s0);
        ret = BVERROR(EINVAL);
        goto end;
    }
    if (!verify_expr(e)) {
    ...
    e->var= bv_mallocz(sizeof(double) *VARS);
    *expr = e;
end:
    bv_free(w);
    return ret;
...
int bv_expr_parse_and_eval(double *d, const char *s,
                           ...
                           void *opaque, int log_offset, void *log_ctx)
{
    ...
    *d = bv_expr_eval(e, const_values, opaque);
    bv_expr_free(e);
    return isnan(*d) ? BVERROR(EINVAL) : 0;
}
...
int main(int argc, char **argv)
{
    ...
    bv_expr_parse_and_eval(&d, "1+(5-2)^(3-1)+1/2+sin(PI)-max(-2.2,-3.1)",
                           const_names, const_values,
                           NULL, NULL, NULL, NULL, NULL, 0, NULL);
    printf("%f == 12.7\n", d);
    bv_expr_parse_and_eval(&d, "80G/80Gi",
                           const_names, const_values,
                           NULL, NULL, NULL, NULL, NULL, 0, NULL);
    ...
    if (argc > 1 && !strcmp(argv[1], "-t")) {
        for (i = 0; i < 1050; i++) {
            ...
            bv_expr_parse_and_eval(&d, "1+(5-2)^(3-1)+1/2+sin(PI)-max(-2.2,-3.1)",
                                   const_names, const_values,
                                   NULL, NULL, NULL, NULL, NULL, 0, NULL);
            STOP_TIMER("bv_expr_parse_and_eval");
        }
    }
