...
static bv_cold void sha ## bits ##_init(void *ctx) \
{                                                  \
    bv_sha_init(ctx, bits);                        \
}

#define DEFINE_SHA512(bits)                        \
static bv_cold void sha ## bits ##_init(void *ctx) \
{                                                  \
    bv_sha512_init(ctx, bits);                     \
}
...
BVHMAC *bv_hmac_alloc(enum BVHMACType type)
{
    ...
    case BV_HMAC_MD5:
        c->blocklen = 64;
        c->hashlen  = 16;
        c->init     = (void*)bv_md5_init;
        c->update   = (void*)bv_md5_update;
        c->final    = (void*)bv_md5_final;
        c->hash     = bv_md5_alloc();
        break;
    case BV_HMAC_SHA1:
        c->blocklen = 64;
        c->hashlen  = 20;
        c->init     = sha160_init;
        c->update   = (void*)bv_sha_update;
        c->final    = (void*)bv_sha_final;
        c->hash     = bv_sha_alloc();
        break;
    case BV_HMAC_SHA224:
        c->blocklen = 64;
    ...
    case BV_HMAC_SHA384:
        ...
        c->hashlen  = 48;
        c->init     = sha384_init;
        c->update   = (void*)bv_sha512_update;
        c->final    = (void*)bv_sha512_final;
        c->hash     = bv_sha512_alloc();
        break;
    case BV_HMAC_SHA512:
        c->blocklen = 128;
        c->hashlen  = 64;
...
void bv_hmac_init(BVHMAC *c, const uint8_t *key, unsigned int keylen)
{
    int i;
...
void bv_hmac_update(BVHMAC *c, const uint8_t *data, unsigned int len)
{
    c->update(c->hash, data, len);
}

int bv_hmac_final(BVHMAC *c, uint8_t *out, unsigned int outlen)
{
    uint8_t block[MAX_BLOCKLEN];
    int i;
    if (outlen < c->hashlen)
...
int bv_hmac_calc(BVHMAC *c, const uint8_t *data, unsigned int len,
                 const uint8_t *key, unsigned int keylen,
                 uint8_t *out, unsigned int outlen)
{
    bv_hmac_init(c, key, keylen);
    bv_hmac_update(c, data, len);
    return bv_hmac_final(c, out, outlen);
}

#ifdef TEST
#include <stdio.h>

static void test(BVHMAC *hmac, const uint8_t *key, int keylen,
                 const uint8_t *data, int datalen)
{
    ...
    if (!data[datalen - 1])
        datalen--;
    out = bv_hmac_calc(hmac, data, datalen, key, keylen, buf, sizeof(buf));
    for (i = 0; i < out; i++)
        printf("%02x", buf[i]);
    printf("\n");
}
...
int main(void)
{
    ...
    enum BVHMACType i = BV_HMAC_SHA224;
    ...
    BVHMAC *hmac = bv_hmac_alloc(BV_HMAC_MD5);
    if (!hmac)
        return 1;
    ...
    /* SHA-1 */
    hmac = bv_hmac_alloc(BV_HMAC_SHA1);
    if (!hmac)
        return 1;
    // RFC 2202 test vectors
    ...
    /* SHA-2 */
    while (i <= BV_HMAC_SHA512) {
        hmac = bv_hmac_alloc(i);
