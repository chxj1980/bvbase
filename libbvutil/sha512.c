...
const int bv_sha512_size = sizeof(BVSHA512);

struct BVSHA512 *bv_sha512_alloc(void)
{
    return bv_mallocz(sizeof(struct BVSHA512));
}
...
#define blk0(i) (block[i] = BV_RB64(buffer + 8 * (i)))
...
bv_cold int bv_sha512_init(BVSHA512 *ctx, int bits)
{
...
void bv_sha512_update(BVSHA512* ctx, const uint8_t* data, unsigned int len)
{
    unsigned int i, j;
...
void bv_sha512_final(BVSHA512* ctx, uint8_t *digest)
{
    uint64_t i = 0;
    uint64_t finalcount = bv_be2ne64(ctx->count << 3);

    bv_sha512_update(ctx, "\200", 1);
    while ((ctx->count & 127) != 112)
        bv_sha512_update(ctx, "", 1);
    bv_sha512_update(ctx, (uint8_t *)&i, 8);
    bv_sha512_update(ctx, (uint8_t *)&finalcount, 8); /* Should cause a transform() */
    for (i = 0; i < ctx->digest_len; i++)
        BV_WB64(digest + i*8, ctx->state[i]);
    if (ctx->digest_len & 1) /* SHA512/224 is 28 bytes, and is not divisible by 8. */
        BV_WB32(digest + i*8, ctx->state[i] >> 32);
}
...
int main(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        for (k = 0; k < 3; k++) {
            bv_sha512_init(&ctx, lengths[j]);
            if (k == 0)
                bv_sha512_update(&ctx, "abc", 3);
            else if (k == 1)
                bv_sha512_update(&ctx, "abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmn"
                                       "hijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu", 112);
            else
                for (i = 0; i < 1000*1000; i++)
                    bv_sha512_update(&ctx, "a", 1);
            bv_sha512_final(&ctx, digest);
