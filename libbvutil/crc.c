...
static struct {
    ...
    uint8_t  bits;
    uint32_t poly;
} bv_crc_table_params[BV_CRC_MAX] = {
    [BV_CRC_8_ATM]      = { 0,  8,       0x07 },
    [BV_CRC_16_ANSI]    = { 0, 16,     0x8005 },
    [BV_CRC_16_CCITT]   = { 0, 16,     0x1021 },
    [BV_CRC_24_IEEE]    = { 0, 24,   0x864CFB },
    [BV_CRC_32_IEEE]    = { 0, 32, 0x04C11DB7 },
    [BV_CRC_32_IEEE_LE] = { 1, 32, 0xEDB88320 },
    [BV_CRC_16_ANSI_LE] = { 1, 16,     0xA001 },
};
static BVCRC bv_crc_table[BV_CRC_MAX][CRC_TABLE_SIZE];
#endif
...
int bv_crc_init(BVCRC *ctx, int le, int bits, uint32_t poly, int ctx_size)
{
    ...
    if (bits < 8 || bits > 32 || poly >= (1LL << bits))
        return -1;
    if (ctx_size != sizeof(BVCRC) * 257 && ctx_size != sizeof(BVCRC) * 1024)
        return -1;
...
const BVCRC *bv_crc_get_table(BVCRCId crc_id)
{
    ...
    if (!bv_crc_table[crc_id][BV_ARRAY_ELEMS(bv_crc_table[crc_id]) - 1])
        if (bv_crc_init(bv_crc_table[crc_id],
                        ...
                        bv_crc_table_params[crc_id].poly,
                        sizeof(bv_crc_table[crc_id])) < 0)
            return NULL;
#endif
    return bv_crc_table[crc_id];
}
                ...
                const uint8_t *buffer, size_t length)
{
    ...
    if (!ctx[256]) {
        ...
        while (buffer < end - 3) {
            crc ^= bv_le2ne32(*(const uint32_t *) buffer); buffer += 4;
...
int main(void)
{
    uint8_t buf[1999];
    int i;
    int p[6][3] = { { BV_CRC_32_IEEE_LE, 0xEDB88320, 0x3D5CDD04 },
                    { BV_CRC_32_IEEE   , 0x04C11DB7, 0xC0F5BAE0 },
                    { BV_CRC_24_IEEE   , 0x864CFB  , 0xB704CE   },
                    { BV_CRC_16_ANSI_LE, 0xA001    , 0xBFD8     },
                    { BV_CRC_16_ANSI   , 0x8005    , 0x1FBB     },
                    { BV_CRC_8_ATM     , 0x07      , 0xE3       }
    };
    const BVCRC *ctx;
    ...
    for (i = 0; i < 6; i++) {
        ctx = bv_crc_get_table(p[i][0]);
        printf("crc %08X = %X\n", p[i][1], bv_crc(ctx, 0, buf, sizeof(buf)));
    }
    return 0;
}
#endif
