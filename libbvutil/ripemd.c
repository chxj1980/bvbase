...
const int bv_ripemd_size = sizeof(BVRIPEMD);

struct BVRIPEMD *bv_ripemd_alloc(void)
{
    return bv_mallocz(sizeof(struct BVRIPEMD));
}
...
bv_cold int bv_ripemd_init(BVRIPEMD *ctx, int bits)
{
...
void bv_ripemd_update(BVRIPEMD* ctx, const uint8_t* data, unsigned int len)
{
    unsigned int i, j;
...
void bv_ripemd_final(BVRIPEMD* ctx, uint8_t *digest)
{
    int i;
    uint64_t finalcount = bv_le2ne64(ctx->count << 3);

    bv_ripemd_update(ctx, "\200", 1);
    while ((ctx->count & 63) != 56)
        bv_ripemd_update(ctx, "", 1);
    bv_ripemd_update(ctx, (uint8_t *)&finalcount, 8); /* Should cause a transform() */
    for (i = 0; i < ctx->digest_len; i++)
        BV_WL32(digest + i*4, ctx->state[i]);
}
...
int main(void)
{
    ...
    for (j = 0; j < 4; j++) {
        ...
        for (k = 0; k < 3; k++) {
            bv_ripemd_init(&ctx, lengths[j]);
            if (k == 0)
                bv_ripemd_update(&ctx, "abc", 3);
            else if (k == 1)
                bv_ripemd_update(&ctx, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", 56);
            else
                for (i = 0; i < 1000*1000; i++)
                    bv_ripemd_update(&ctx, "a", 1);
            bv_ripemd_final(&ctx, digest);
