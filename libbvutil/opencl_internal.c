...
int bvpriv_opencl_set_parameter(FFOpenclParam *opencl_param, ...)
{
    ...
    if (!opencl_param->kernel) {
        bv_log(opencl_param->ctx, BV_LOG_ERROR, "OpenCL kernel must be set\n");
    ...
    do {
        ...
        if (!param_size) {
            bv_log(opencl_param->ctx, BV_LOG_ERROR, "Parameter size must not be 0\n");
            ret = BVERROR(EINVAL);
            goto end;
        }
        status = clSetKernelArg(opencl_param->kernel, opencl_param->param_num, param_size, param);
        if (status != CL_SUCCESS) {
            bv_log(opencl_param->ctx, BV_LOG_ERROR, "Cannot set kernel argument: %s\n", bv_opencl_errstr(status));
