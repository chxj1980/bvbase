...
static const struct {
    ...
    { "mono",        1,  BV_CH_LAYOUT_MONO },
    { "stereo",      2,  BV_CH_LAYOUT_STEREO },
    { "2.1",         3,  BV_CH_LAYOUT_2POINT1 },
    { "3.0",         3,  BV_CH_LAYOUT_SURROUND },
    { "3.0(back)",   3,  BV_CH_LAYOUT_2_1 },
    { "4.0",         4,  BV_CH_LAYOUT_4POINT0 },
    { "quad",        4,  BV_CH_LAYOUT_QUAD },
    { "quad(side)",  4,  BV_CH_LAYOUT_2_2 },
    { "3.1",         4,  BV_CH_LAYOUT_3POINT1 },
    { "5.0",         5,  BV_CH_LAYOUT_5POINT0_BACK },
    { "5.0(side)",   5,  BV_CH_LAYOUT_5POINT0 },
    { "4.1",         5,  BV_CH_LAYOUT_4POINT1 },
    { "5.1",         6,  BV_CH_LAYOUT_5POINT1_BACK },
    { "5.1(side)",   6,  BV_CH_LAYOUT_5POINT1 },
    { "6.0",         6,  BV_CH_LAYOUT_6POINT0 },
    { "6.0(front)",  6,  BV_CH_LAYOUT_6POINT0_FRONT },
    { "hexagonal",   6,  BV_CH_LAYOUT_HEXAGONAL },
    { "6.1",         7,  BV_CH_LAYOUT_6POINT1 },
    { "6.1",         7,  BV_CH_LAYOUT_6POINT1_BACK },
    { "6.1(front)",  7,  BV_CH_LAYOUT_6POINT1_FRONT },
    { "7.0",         7,  BV_CH_LAYOUT_7POINT0 },
    { "7.0(front)",  7,  BV_CH_LAYOUT_7POINT0_FRONT },
    { "7.1",         8,  BV_CH_LAYOUT_7POINT1 },
    { "7.1(wide)",   8,  BV_CH_LAYOUT_7POINT1_WIDE_BACK },
    { "7.1(wide-side)",   8,  BV_CH_LAYOUT_7POINT1_WIDE },
    { "octagonal",   8,  BV_CH_LAYOUT_OCTAGONAL },
...
#endif
{
    ...
    if (compat) {
        if (end - name == name_len ||
            ...
            layout = bv_get_default_channel_layout(i);
            if (end - name == name_len) {
...
void bv_bprint_channel_layout(struct BVBPrint *bp,
                              int nb_channels, uint64_t channel_layout)
{
    ...
    bv_bprintf(bp, "%d channels", nb_channels);
    if (channel_layout) {
        int i, ch;
        bv_bprintf(bp, " (");
        for (i = 0, ch = 0; i < 64; i++) {
...
void bv_get_channel_layout_string(char *buf, int buf_size,
                                  int nb_channels, uint64_t channel_layout)
{
    BVBPrint bp;

    bv_bprint_init_for_buffer(&bp, buf, buf_size);
    bv_bprint_channel_layout(&bp, nb_channels, channel_layout);
}

int bv_get_channel_layout_nb_channels(uint64_t channel_layout)
{
    return bv_popcount64(channel_layout);
}
                                        ...
                                        uint64_t channel)
{
    if (!(channel_layout & channel) ||
        bv_get_channel_layout_nb_channels(channel) != 1)
        return BVERROR(EINVAL);
    channel_layout &= channel - 1;
    return bv_get_channel_layout_nb_channels(channel_layout);
}
...
uint64_t bv_channel_layout_extract_channel(uint64_t channel_layout, int index)
{
    ...
    if (bv_get_channel_layout_nb_channels(channel_layout) <= index)
        return 0;
