...
typedef struct BVAES {
    // Note: round_key[16] is accessed in the init code, but this only
    // overwrites state, which does not matter (see also commit ba554c0).
    bv_aes_block round_key[15];
    bv_aes_block state[2];
    int rounds;
} BVAES;

const int bv_aes_size= sizeof(BVAES);

struct BVAES *bv_aes_alloc(void)
{
    return bv_mallocz(sizeof(struct BVAES));
}
...
static inline void addkey(bv_aes_block *dst, const bv_aes_block *src,
                          const bv_aes_block *round_key)
{
    dst->u64[0] = src->u64[0] ^ round_key->u64[0];
    dst->u64[1] = src->u64[1] ^ round_key->u64[1];
}

static inline void addkey_s(bv_aes_block *dst, const uint8_t *src,
                            const bv_aes_block *round_key)
{
    dst->u64[0] = BV_RN64(src)     ^ round_key->u64[0];
    dst->u64[1] = BV_RN64(src + 8) ^ round_key->u64[1];
}

static inline void addkey_d(uint8_t *dst, const bv_aes_block *src,
                            const bv_aes_block *round_key)
{
    BV_WN64(dst,     src->u64[0] ^ round_key->u64[0]);
    BV_WN64(dst + 8, src->u64[1] ^ round_key->u64[1]);
}

static void subshift(bv_aes_block s0[2], int s, const uint8_t *box)
{
    bv_aes_block *s1 = (bv_aes_block *) (s0[0].u8 - s);
    bv_aes_block *s3 = (bv_aes_block *) (s0[0].u8 + s);
...
static inline void mix(bv_aes_block state[2], uint32_t multbl[][256], int s1, int s3){
...
static inline void crypt(BVAES *a, int s, const uint8_t *sbox,
                         uint32_t multbl[][256])
{
    int r;
...
void bv_aes_crypt(BVAES *a, uint8_t *dst, const uint8_t *src,
...
int bv_aes_init(BVAES *a, const uint8_t *key, int key_bits, int decrypt)
{
    ...
    if (!enc_multbl[BV_ARRAY_ELEMS(enc_multbl)-1][BV_ARRAY_ELEMS(enc_multbl[0])-1]) {
        j = 1;
        for (i = 0; i < 255; i++) {
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < 2; i++) {
        bv_aes_init(&b, rkey[i], 128, 1);
        bv_aes_crypt(&b, temp, rct[i], 1, NULL, 1);
        for (j = 0; j < 16; j++) {
            if (rpt[i][j] != temp[j]) {
                bv_log(NULL, BV_LOG_ERROR, "%d %02X %02X\n",
                       j, rpt[i][j], temp[j]);
                err = 1;
            }
        }
    ...
    if (argc > 1 && !strcmp(argv[1], "-t")) {
        ...
        bv_aes_init(&ae, "PI=3.141592654..", 128, 0);
        bv_aes_init(&ad, "PI=3.141592654..", 128, 1);
        bv_lfg_init(&prng, 1);

        for (i = 0; i < 10000; i++) {
            for (j = 0; j < 16; j++) {
                pt[j] = bv_lfg_get(&prng);
            }
            {
                START_TIMER;
                bv_aes_crypt(&ae, temp, pt, 1, NULL, 0);
                if (!(i & (i - 1)))
                    bv_log(NULL, BV_LOG_ERROR, "%02X %02X %02X %02X\n",
                           temp[0], temp[5], temp[10], temp[15]);
                bv_aes_crypt(&ad, temp, temp, 1, NULL, 1);
                STOP_TIMER("aes");
            }
            for (j = 0; j < 16; j++) {
                if (pt[j] != temp[j]) {
                    bv_log(NULL, BV_LOG_ERROR, "%d %d %02X %02X\n",
                           i, j, pt[j], temp[j]);
                }
            }
        }
