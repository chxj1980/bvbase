...
#if BV_CONFIG_SMALL || defined(GENTABLES)
 ...
 * It can be regenerated by compiling this file with -DBV_CONFIG_SMALL -DTEST -DGENTABLES
 */
...
int bv_des_init(BVDES *d, const uint8_t *key, int key_bits, bv_unused int decrypt) {
    if (key_bits != 64 && key_bits != 192)
        return -1;
    d->triple_des = key_bits > 64;
    gen_roundkeys(d->round_keys[0], BV_RB64(key));
    if (d->triple_des) {
        gen_roundkeys(d->round_keys[1], BV_RB64(key +  8));
        gen_roundkeys(d->round_keys[2], BV_RB64(key + 16));
    }
    return 0;
}

static void bv_des_crypt_mac(BVDES *d, uint8_t *dst, const uint8_t *src, int count, uint8_t *iv, int decrypt, int mac) {
    uint64_t iv_val = iv ? BV_RB64(iv) : 0;
    while (count-- > 0) {
        uint64_t dst_val;
        uint64_t src_val = src ? BV_RB64(src) : 0;
        if (decrypt) {
...
void bv_des_crypt(BVDES *d, uint8_t *dst, const uint8_t *src, int count, uint8_t *iv, int decrypt) {
    bv_des_crypt_mac(d, dst, src, count, iv, decrypt, 0);
}

void bv_des_mac(BVDES *d, uint8_t *dst, const uint8_t *src, int count) {
    bv_des_crypt_mac(d, dst, src, count, (uint8_t[8]){0}, 0, 1);
}
...
static const BV_DECLARE_ALIGNED(8, uint8_t, plain)[] = {0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10};
static const BV_DECLARE_ALIGNED(8, uint8_t, crypt)[] = {0x4a, 0xb6, 0x5b, 0x3d, 0x4b, 0x06, 0x15, 0x18};
static BV_DECLARE_ALIGNED(8, uint8_t, tmp)[8];
static BV_DECLARE_ALIGNED(8, uint8_t, large_buffer)[10002][8];
...
static int run_test(int cbc, int decrypt) {
    ...
    uint64_t res;
    BV_WB64(large_buffer[0], 0x4e6f772069732074ULL);
    BV_WB64(large_buffer[1], 0x1234567890abcdefULL);
    BV_WB64(tmp,             0x1234567890abcdefULL);
    bv_des_init(&d, cbc_key, 192, decrypt);
    bv_des_crypt(&d, large_buffer[delay], large_buffer[0], 10000, cbc ? tmp : NULL, decrypt);
    res = BV_RB64(large_buffer[9999 + delay]);
    if (cbc) {
        if (decrypt)
...
int main(void) {
    ...
    srand(bv_gettime());
    key[0] = BV_RB64(test_key);
    data = BV_RB64(plain);
    gen_roundkeys(roundkeys, key[0]);
    if (des_encdec(data, roundkeys, 0) != BV_RB64(crypt)) {
        printf("Test 1 failed\n");
        return 1;
    }
    bv_des_init(&d, test_key, 64, 0);
    bv_des_crypt(&d, tmp, plain, 1, NULL, 0);
    ...
    for (i = 0; i < 1000; i++) {
        ...
        data = rand64();
        bv_des_init(&d, (uint8_t*)key, 192, 0);
        bv_des_crypt(&d, (uint8_t*)&ct, (uint8_t*)&data, 1, NULL, 0);
        bv_des_init(&d, (uint8_t*)key, 192, 1);
        bv_des_crypt(&d, (uint8_t*)&ct, (uint8_t*)&ct, 1, NULL, 1);
        if (ct != data) {
