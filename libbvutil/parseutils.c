...
#define bv_get_random_seed bv_get_random_seed_deterministic
static uint32_t bv_get_random_seed_deterministic(void);

#define time(t) 1331972053

#endif

int bv_parse_ratio(BVRational *q, const char *str, int max,
                   int log_offset, void *log_ctx)
{
    ...
    if (sscanf(str, "%d:%d%c", &q->num, &q->den, &c) != 2) {
        double d;
        ret = bv_expr_parse_and_eval(&d, str, NULL, NULL,
                                     NULL, NULL, NULL, NULL,
        ...
        *q = bv_d2q(d, max);
        ...
        bv_reduce(&q->num, &q->den, q->num, q->den, max);
    }
...
int bv_parse_video_rate(BVRational *rate, const char *arg)
{
    ...
    if ((ret = bv_parse_ratio_quiet(rate, arg, 1001000)) < 0)
        return ret;
...
{
    return bv_strcasecmp(lhs, ((const ColorEntry *)rhs)->name);
}

#define ALPHA_SEP '@'

int bv_parse_color(uint8_t *rgba_color, const char *color_string, int slen,
                   void *log_ctx)
{
    ...
    if (!bv_strcasecmp(color_string2, "random") || !bv_strcasecmp(color_string2, "bikeshed")) {
        int rgba = bv_get_random_seed();
        ...
        if (*tail || (len != 6 && len != 8)) {
            bv_log(log_ctx, BV_LOG_ERROR, "Invalid 0xRRGGBB[AA] color string: '%s'\n", color_string2);
            return BVERROR(EINVAL);
        }
        if (len == 8) {
            rgba_color[3] = rgba;
        ...
        entry = bsearch(color_string2,
                        ...
                        color_table_compare);
        if (!entry) {
            bv_log(log_ctx, BV_LOG_ERROR, "Cannot find color '%s'\n", color_string2);
            return BVERROR(EINVAL);
        }
    ...
    if (tail) {
        ...
        if (tail == alpha_string || *tail || alpha > 255 || alpha < 0) {
            bv_log(log_ctx, BV_LOG_ERROR, "Invalid alpha value specifier '%s' in '%s'\n",
...
const char *bv_get_known_color_name(int color_idx, const uint8_t **rgbp)
{
...
char *bv_small_strptime(const char *p, const char *fmt, struct tm *dt)
{
    int c, val;

    for(;;) {
        /* consume time string until a non whitespace char is found */
        while (bv_isspace(*fmt)) {
            while (bv_isspace(*p))
                p++;
            fmt++;
        }
...
int bv_parse_time(int64_t *timeval, const char *timestr, int duration)
{
    const char *p, *q;
    int64_t t;
    time_t now;
    ...
    *timeval = INT64_MIN;
    if (!duration) {
        ...
        q = bv_small_strptime(p, "%J:%M:%S", &dt);
        if (!q) {
            /* parse timestr as MM:SS */
            q = bv_small_strptime(p, "%M:%S", &dt);
            dt.tm_hour = 0;
        }
        if (!q) {
            char *o;
    ...
    if (duration) {
        ...
        t = is_utc ? bv_timegm(&dt) : mktime(&dt);
    }
...
int bv_find_info_tag(char *arg, int arg_size, const char *tag1, const char *info)
{
    const char *p;
    char tag[128], *q;
...
int main(void)
{
    printf("Testing bv_parse_video_rate()\n");
    {
        int i;
    ...
    printf("\nTesting bv_small_strptime()\n");
    {
        ...
        for (i = 0;  i < FF_ARRAY_ELEMS(fmt_timespec_entries); i++) {
            ...
            p = bv_small_strptime(e->timespec, e->fmt, &tm);
            if (p) {
