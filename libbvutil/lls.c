...
bv_cold void bvpriv_init_lls(LLSModel *m, int indep_count)
{
...
int main(void)
{
    ...
    for (i = 0; i < 100; i++) {
        ...
        var[0] = (bv_lfg_get(&lfg) / (double) UINT_MAX - 0.5) * 2;
        var[1] = var[0] + bv_lfg_get(&lfg) / (double) UINT_MAX - 0.5;
        var[2] = var[1] + bv_lfg_get(&lfg) / (double) UINT_MAX - 0.5;
        var[3] = var[2] + bv_lfg_get(&lfg) / (double) UINT_MAX - 0.5;
        m.update_lls(&m, var);
