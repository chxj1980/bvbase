...
int bv_base64_decode(uint8_t *out, const char *in_str, int out_size)
{
    uint8_t *dst = out;
    ...
    while (end - dst > 3) {
        ...
        BASE64_DEC_STEP(2);
        BASE64_DEC_STEP(3);
        // Using BV_WB32 directly confuses compiler
        v = bv_be2ne32(v << 8);
        BV_WN32(dst, v);
        dst += 3;
        in += 4;
...
out1:
out0:
    return bits & 1 ? BVERROR_INVALIDDATA : dst - out;
}
...
char *bv_base64_encode(char *out, int out_size, const uint8_t *in, int in_size)
{
    ...
    if (in_size >= UINT_MAX / 4 ||
        out_size < BV_BASE64_SIZE(in_size))
        return NULL;
    ret = dst = out;
                              ...
                              const char *encoded_ref)
{
    ...
    if (!bv_base64_encode(encoded, MAX_ENCODED_SIZE, data, data_size)) {
    ...
    if ((data2_size = bv_base64_decode(data2, encoded, max_data2_size)) != data_size) {
        printf("Failed: cannot decode the encoded string\n"
               "Encoded:\n%s\n", encoded);
        return 1;
    }
    if ((data2_size = bv_base64_decode(data2, encoded, data_size)) != data_size) {
    ...
    if (bv_base64_decode(NULL, encoded, 0) != 0) {
...
int main(int argc, char ** argv)
{
    ...
    if (argc>1 && !strcmp(argv[1], "-t")) {
        ...
        for(i=0; i<10000; i++){
            START_TIMER
            bv_base64_encode(out, sizeof(out), in, sizeof(in));
            STOP_TIMER("encode")
        }
        for(i=0; i<10000; i++){
            START_TIMER
            bv_base64_decode(in, out, sizeof(in));
            STOP_TIMER("decode")
        }

        for(i=0; i<10000; i++){
            START_TIMER
            bv_base64_decode(NULL, out, 0);
