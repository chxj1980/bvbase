...
static int get_cpuinfo(uint32_t *hwcap)
{
    ...
    *hwcap = 0;
    while (fgets(buf, sizeof(buf), f)) {
        if (bv_strstart(buf, "Features", NULL)) {
            if (strstr(buf, " edsp "))
                *hwcap |= HWCAP_EDSP;
            if (strstr(buf, " tls "))
                *hwcap |= HWCAP_TLS;
...
#define check_cap(cap, flag) do {               \
    ...
    /* The v6 checks above are not reliable so let higher flags
       trickle down. */
    if (flags & (BV_CPU_FLAG_VFPV3 | BV_CPU_FLAG_NEON))
        flags |= BV_CPU_FLAG_ARMV6T2;
    else
     ...
     * are only enabled on ARMv6. */
        flags |= BV_CPU_FLAG_SETEND;

    if (flags & BV_CPU_FLAG_ARMV6T2)
        flags |= BV_CPU_FLAG_ARMV6;
