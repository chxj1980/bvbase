...
PCA *ff_pca_init(int n){
    ...
    pca= bv_mallocz(sizeof(*pca));
    pca->n= n;
    pca->z = bv_malloc_array(n, sizeof(*pca->z));
    pca->count=0;
    pca->covariance= bv_calloc(n*n, sizeof(double));
    pca->mean= bv_calloc(n, sizeof(double));
...
#define LEN 8
    ...
    for(i=0; i<9000000; i++){
        double v[2*LEN+100];
        double sum=0;
        int pos = bv_lfg_get(&prng) % LEN;
        int v2  = bv_lfg_get(&prng) % 101 - 50;
        v[0]    = bv_lfg_get(&prng) % 101 - 50;
        for(j=1; j<8; j++){
            if(j<=pos) v[j]= v[0];
            else       v[j]= v2;
            sum += v[j];
