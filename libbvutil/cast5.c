...
typedef struct BVCAST5 {
    uint32_t Km[17];
    uint32_t Kr[17];
    int rounds;
} BVCAST5;

const int bv_cast5_size = sizeof(BVCAST5);
...
static void encipher(BVCAST5* cs, uint8_t* dst, const uint8_t* src)
{
    uint32_t r, l, f, I;
    l = BV_RB32(src);
    r = BV_RB32(src + 4);
...
static void decipher(BVCAST5* cs, uint8_t* dst, const uint8_t* src, uint8_t *iv)
{
    uint32_t f, I, r, l;
    l = BV_RB32(src);
    r = BV_RB32(src + 4);
...
{
    return bv_mallocz(sizeof(struct BVCAST5));
}

bv_cold int bv_cast5_init(BVCAST5* cs, const uint8_t *key, int key_bits)
{
    uint8_t newKey[16];
    int i;
    uint32_t p[4], q[4];
...
void bv_cast5_crypt2(BVCAST5* cs, uint8_t* dst, const uint8_t* src, int count, uint8_t *iv, int decrypt)
{
    int i;
    while (count--) {
        if (decrypt) {
...
void bv_cast5_crypt(BVCAST5* cs, uint8_t* dst, const uint8_t* src, int count, int decrypt)
{
    while (count--) {
        if (decrypt){
...
int main(int argc, char** argv)
{
    ...
    for (j = 0; j < 3; j++){
        ...
        bv_cast5_init(cs, Key[j], key_bits[j]);
        bv_cast5_crypt(cs, temp, rpt, 1, 0);
        for (i = 0;i < 8; i++){
            if (rct[j][i] != temp[i]){
        ...
        bv_cast5_crypt(cs, temp, rct[j], 1, 1);
        for (i =0; i < 8; i++) {
            if (rpt[i] != temp[i]) {
                bv_log(NULL, BV_LOG_ERROR, "%d %02x %02x\n", i, rpt[i], temp[i]);
                err = 1;
    ...
    for (i = 0; i < 1000000; i++){
        bv_cast5_init(cs, rpt2[1], 128);
        bv_cast5_crypt(cs, rpt2[0], rpt2[0], 2, 0);
        bv_cast5_init(cs, rpt2[0], 128);
        bv_cast5_crypt(cs, rpt2[1], rpt2[1], 2, 0);
    }
    for (j = 0; j < 2; j++) {
        for (i = 0; i < 16; i++) {
    ...
    for (j = 0; j < 3; j++) {
        ...
        bv_cast5_init(cs, Key[j], key_bits[j]);
        memcpy(temp, iv, 8);
        bv_cast5_crypt2(cs, rpt2[0], rct2[0], 2, temp, 0);
        memcpy(temp, iv, 8);
        bv_cast5_crypt2(cs, rpt2[0], rpt2[0], 2, temp, 1);
        for (i = 0; i < 16; i++) {
