...
int main(int argc, const char *argv[])
{
    ...
    if (bv_config_get_video_encoder(config_ctx, 0, 2, &video_encoder) < 0) {
...
#if 0
    video_encoder.codec_context.codec_id = BV_CODEC_ID_JPEG;
    video_encoder.codec_context.bit_rate = 512;

    if (bv_config_set_video_encoder(config_ctx, 0, 2, &video_encoder) < 0) {
