...
#include "libbvutil/intreadwrite.h"
...
#if BV_HAVE_PTHREAD_CANCEL
#include <pthread.h>
#endif

#ifndef BV_HAVE_PTHREAD_CANCEL
#define BV_HAVE_PTHREAD_CANCEL 0
#endif

#ifndef IPV6_ADD_MEMBERSHIP
#define IPV6_ADD_MEMBERSHIP IPV6_JOIN_GROUP
#define IPV6_DROP_MEMBERSHIP IPV6_LEBVE_GROUP
#endif
                                 ...
                                 struct sockaddr *addr)
{
    ...
    if (addr->sa_family == AF_INET) {
        if (setsockopt(sockfd, IPPROTO_IP, IP_MULTICAST_TTL, &mcastTTL, sizeof(mcastTTL)) < 0) {
            log_net_error(NULL, BV_LOG_ERROR, "setsockopt(IP_MULTICAST_TTL)");
            return -1;
        }
    }
#endif
    ...
    if (addr->sa_family == AF_INET6) {
        if (setsockopt(sockfd, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &mcastTTL, sizeof(mcastTTL)) < 0) {
            log_net_error(NULL, BV_LOG_ERROR, "setsockopt(IPV6_MULTICAST_HOPS)");
            return -1;
        }
    }
#endif
...
static int udp_join_multicast_group(int sockfd, struct sockaddr *addr,struct sockaddr *local_addr)
{
    ...
    if (addr->sa_family == AF_INET) {
        ...
        if (setsockopt(sockfd, IPPROTO_IP, IP_ADD_MEMBERSHIP, (const void *)&mreq, sizeof(mreq)) < 0) {
            log_net_error(NULL, BV_LOG_ERROR, "setsockopt(IP_ADD_MEMBERSHIP)");
            return -1;
        }
    }
#endif
#if BV_HAVE_STRUCT_IPV6_MREQ && defined(IPPROTO_IPV6)
    if (addr->sa_family == AF_INET6) {
        ...
        if (setsockopt(sockfd, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &mreq6, sizeof(mreq6)) < 0) {
            log_net_error(NULL, BV_LOG_ERROR, "setsockopt(IPV6_ADD_MEMBERSHIP)");
            return -1;
        }
    }
#endif
...
static int udp_leave_multicast_group(int sockfd, struct sockaddr *addr,struct sockaddr *local_addr)
{
    ...
    if (addr->sa_family == AF_INET) {
        ...
        if (setsockopt(sockfd, IPPROTO_IP, IP_DROP_MEMBERSHIP, (const void *)&mreq, sizeof(mreq)) < 0) {
            log_net_error(NULL, BV_LOG_ERROR, "setsockopt(IP_DROP_MEMBERSHIP)");
            return -1;
        }
    }
#endif
    ...
    if (addr->sa_family == AF_INET6) {
        ...
        if (setsockopt(sockfd, IPPROTO_IPV6, IPV6_DROP_MEMBERSHIP, &mreq6, sizeof(mreq6)) < 0) {
            log_net_error(NULL, BV_LOG_ERROR, "setsockopt(IPV6_DROP_MEMBERSHIP)");
            return -1;
        }
    }
#endif
                                         ...
                                         int type, int family, int flags)
{
    ...
    if ((error = getaddrinfo(node, service, &hints, &res))) {
        res = NULL;
        bv_log(NULL, BV_LOG_ERROR, "udp_resolve_host: %s\n", gai_strerror(error));
    }
...
{
#if BV_HAVE_STRUCT_GROUP_SOURCE_REQ && defined(MCAST_BLOCK_SOURCE) && !defined(_WIN32)
    ...
    for (i = 0; i < nb_sources; i++) {
        ...
        if (sourceaddr->ai_addr->sa_family != AF_INET) {
            freeaddrinfo(sourceaddr);
            bv_log(NULL, BV_LOG_ERROR, "%s is of incorrect protocol family\n",
                   sources[i]);
            return BVERROR(EINVAL);
        }
...
static int udp_port(struct sockaddr_storage *addr, int addr_len)
{
    ...
    if ((error = getnameinfo((struct sockaddr *)addr, addr_len, NULL, 0,  sbuf, sizeof(sbuf), NI_NUMERICSERV)) != 0) {
        bv_log(NULL, BV_LOG_ERROR, "getnameinfo: %s\n", gai_strerror(error));
        return -1;
    }
...
/**
 * If no filename is given to bv_open_input_file because you want to
...
static int bv_udp_set_remote_url(BVURLContext *h, const char *uri)
{
    ...
    bv_url_split(NULL, 0, NULL, 0, hostname, sizeof(hostname), &port, NULL, 0, uri);
        ...
        if (bv_find_info_tag(buf, sizeof(buf), "connect", p)) {
            ...
            if (s->is_connected && !was_connected) {
                if (connect(s->udp_fd, (struct sockaddr *) &s->dest_addr,
                            s->dest_addr_len)) {
                    s->is_connected = 0;
                    log_net_error(h, BV_LOG_ERROR, "connect");
                    return BVERROR(EIO);
                }
            }
        }
 ...
 * @param h media file context
 */
static int udp_get_file_handle(BVURLContext *h)
{
    UDPContext *s = h->priv_data;
    return s->udp_fd;
}
...
#if BV_HAVE_PTHREAD_CANCEL
static void *circular_buffer_task( void *_BVURLContext)
{
    BVURLContext *h = _BVURLContext;
    UDPContext *s = h->priv_data;
    int old_cancelstate;
    ...
    while(1) {
        ...
        if(bv_fifo_space(s->fifo) < len + 4) {
            /* No Space left */
            if (s->overrun_nonfatal) {
                bv_log(h, BV_LOG_WARNING, "Circular buffer overrun. "
                        "Surviving due to overrun_nonfatal option\n");
                continue;
            } else {
                bv_log(h, BV_LOG_ERROR, "Circular buffer overrun. "
        ...
        bv_fifo_generic_write(s->fifo, s->tmp, len+4, NULL);
                             ...
                             int max_sources)
{
    ...
    while (1) {
        ...
        if (next)
            *next = '\0';
        sources[*num_sources] = bv_strdup(source_start);
...
static int udp_open(BVURLContext *h, const char *uri, int flags, BVDictionary **options)
{
    ...
    is_output = !(flags & BV_IO_FLAG_READ);
    if (!s->buffer_size) /* if not set explicitly */
        s->buffer_size = is_output ? UDP_TX_BUF_SIZE : UDP_MAX_PKT_SIZE;

    p = strchr(uri, '?');
    if (p) {
        if (bv_find_info_tag(buf, sizeof(buf), "reuse", p)) {
            char *endptr = NULL;
        ...
        if (bv_find_info_tag(buf, sizeof(buf), "overrun_nonfatal", p)) {
            char *endptr = NULL;
        ...
        if (bv_find_info_tag(buf, sizeof(buf), "ttl", p)) {
            s->ttl = strtol(buf, NULL, 10);
        }
        if (bv_find_info_tag(buf, sizeof(buf), "udplite_coverage", p)) {
            s->udplite_coverage = strtol(buf, NULL, 10);
        }
        if (bv_find_info_tag(buf, sizeof(buf), "localport", p)) {
            s->local_port = strtol(buf, NULL, 10);
        }
        if (bv_find_info_tag(buf, sizeof(buf), "pkt_size", p)) {
            s->packet_size = strtol(buf, NULL, 10);
        }
        if (bv_find_info_tag(buf, sizeof(buf), "buffer_size", p)) {
            s->buffer_size = strtol(buf, NULL, 10);
        }
        if (bv_find_info_tag(buf, sizeof(buf), "connect", p)) {
            s->is_connected = strtol(buf, NULL, 10);
        }
        if (bv_find_info_tag(buf, sizeof(buf), "dscp", p)) {
            dscp = strtol(buf, NULL, 10);
        }
        if (bv_find_info_tag(buf, sizeof(buf), "fifo_size", p)) {
        ...
        if (bv_find_info_tag(buf, sizeof(buf), "localaddr", p)) {
            bv_strlcpy(localaddr, buf, sizeof(localaddr));
        }
        if (bv_find_info_tag(buf, sizeof(buf), "sources", p)) {
            if (parse_source_list(buf, include_sources, &num_include_sources,
                                  BV_ARRAY_ELEMS(include_sources)))
                goto fail;
        }
        if (bv_find_info_tag(buf, sizeof(buf), "block", p)) {
            if (parse_source_list(buf, exclude_sources, &num_exclude_sources,
                                  BV_ARRAY_ELEMS(exclude_sources)))
                goto fail;
        }
        if (!is_output && bv_find_info_tag(buf, sizeof(buf), "timeout", p))
            s->timeout = strtol(buf, NULL, 10);
        if (is_output && bv_find_info_tag(buf, sizeof(buf), "broadcast", p))
            s->is_broadcast = strtol(buf, NULL, 10);
    }
    /* handling needed to support options picking from both BVOption and URL */
    ...
    if ((s->is_multicast || !s->local_port) && (h->flags & BV_IO_FLAG_READ))
        s->local_port = port;
    ...
    if (s->udplite_coverage) {
        if (setsockopt (udp_fd, IPPROTO_UDPLITE, UDPLITE_SEND_CSCOV, &(s->udplite_coverage), sizeof(s->udplite_coverage)) != 0)
            bv_log(h, BV_LOG_WARNING, "socket option UDPLITE_SEND_CSCOV not available");

        if (setsockopt (udp_fd, IPPROTO_UDPLITE, UDPLITE_RECV_CSCOV, &(s->udplite_coverage), sizeof(s->udplite_coverage)) != 0)
            bv_log(h, BV_LOG_WARNING, "socket option UDPLITE_RECV_CSCOV not available");
    }
    ...
    if (s->is_multicast && !(h->flags & BV_IO_FLAG_WRITE)) {
    ...
    if (bind_ret < 0 && bind(udp_fd,(struct sockaddr *)&my_addr, len) < 0) {
        log_net_error(h, BV_LOG_ERROR, "bind failed");
        goto fail;
    }
    ...
    if (s->is_multicast) {
        ...
        if (h->flags & BV_IO_FLAG_READ) {
            ...
            if (num_include_sources && num_exclude_sources) {
                bv_log(h, BV_LOG_ERROR, "Simultaneously including and excluding multicast sources is not supported\n");
                goto fail;
            }
    ...
    if (is_output) {
        ...
        if (setsockopt(udp_fd, SOL_SOCKET, SO_SNDBUF, &tmp, sizeof(tmp)) < 0) {
            log_net_error(h, BV_LOG_ERROR, "setsockopt(SO_SNDBUF)");
            goto fail;
        }
    } else {
        ...
        if (setsockopt(udp_fd, SOL_SOCKET, SO_RCVBUF, &tmp, sizeof(tmp)) < 0) {
            log_net_error(h, BV_LOG_WARNING, "setsockopt(SO_RECVBUF)");
        }
        len = sizeof(tmp);
        if (getsockopt(udp_fd, SOL_SOCKET, SO_RCVBUF, &tmp, &len) < 0) {
            log_net_error(h, BV_LOG_WARNING, "getsockopt(SO_RCVBUF)");
        } else {
            bv_log(h, BV_LOG_DEBUG, "end receive buffer size reported is %d\n", tmp);
            if(tmp < s->buffer_size)
                bv_log(h, BV_LOG_WARNING, "attempted to set receive buffer to size %d but it only ended up set as %d", s->buffer_size, tmp);
        }
    ...
    for (i = 0; i < num_include_sources; i++)
        bv_freep(&include_sources[i]);
    for (i = 0; i < num_exclude_sources; i++)
        bv_freep(&exclude_sources[i]);
    ...
    if (!is_output && s->circular_buffer_size) {
        ...
        s->fifo = bv_fifo_alloc(s->circular_buffer_size);
        ret = pthread_mutex_init(&s->mutex, NULL);
        if (ret != 0) {
            bv_log(h, BV_LOG_ERROR, "pthread_mutex_init failed : %s\n", strerror(ret));
            goto fail;
        }
        ret = pthread_cond_init(&s->cond, NULL);
        if (ret != 0) {
            bv_log(h, BV_LOG_ERROR, "pthread_cond_init failed : %s\n", strerror(ret));
            goto cond_fail;
        }
        ret = pthread_create(&s->circular_buffer_thread, NULL, circular_buffer_task, h);
        if (ret != 0) {
            bv_log(h, BV_LOG_ERROR, "pthread_create failed : %s\n", strerror(ret));
            goto thread_fail;
        }
        s->thread_started = 1;
    }
...
static int udp_read(BVURLContext *h, uint8_t *buf, size_t size)
{
    ...
    int ret;
#if BV_HAVE_PTHREAD_CANCEL
    int avail, nonblock = h->flags & BV_IO_FLAG_NONBLOCK;

    if (s->fifo) {
        pthread_mutex_lock(&s->mutex);
        do {
            avail = bv_fifo_size(s->fifo);
            if (avail) { // >=size) {
                uint8_t tmp[4];

                bv_fifo_generic_read(s->fifo, tmp, 4, NULL);
                avail= BV_RL32(tmp);
                if(avail > size){
                    bv_log(h, BV_LOG_WARNING, "Part of datagram lost due to insufficient buffer size\n");
                    avail= size;
                }

                bv_fifo_generic_read(s->fifo, buf, avail, NULL);
                bv_fifo_drain(s->fifo, BV_RL32(tmp) - avail);
                pthread_mutex_unlock(&s->mutex);
                return avail;
                ...
                int64_t t = bv_gettime() + 100000;
                struct timespec tv = { .tv_sec  =  t / 1000000,
                                       .tv_nsec = (t % 1000000) * 1000 };
                if (pthread_cond_timedwait(&s->cond, &s->mutex, &tv) < 0) {
                    pthread_mutex_unlock(&s->mutex);
                    return BVERROR(errno == ETIMEDOUT ? EAGAIN : errno);
                }
                nonblock = 1;
            }
        } while( 1);
...
static int udp_close(BVURLContext *h)
{
    ...
    if (s->is_multicast && (h->flags & BV_IO_FLAG_READ))
    ...
    if (s->thread_started) {
        ...
        if (ret != 0)
            bv_log(h, BV_LOG_ERROR, "pthread_join(): %s\n", strerror(ret));
        pthread_mutex_destroy(&s->mutex);
        pthread_cond_destroy(&s->cond);
    }
#endif
