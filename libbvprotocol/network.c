...
#if BV_HAVE_THREADS && GNUTLS_VERSION_NUMBER <= 0x020b00
#include <gcrypt.h>
#include <errno.h>
...
int bb_tls_init(void)
{
    ...
    if (!openssl_init) {
        ...
        if (!CRYPTO_get_locking_callback()) {
            int i;
            openssl_mutexes = bv_malloc_array(sizeof(pthread_mutex_t), CRYPTO_num_locks());
...
int bb_network_wait_fd_timeout(int fd, int write, int64_t timeout, BVIOInterruptCB *int_cb)
{
    ...
    while (1) {
        ...
        if (timeout > 0) {
            if (!wait_start)
                wait_start = bv_gettime_relative();
            else if (bv_gettime_relative() - wait_start > timeout)
                return BVERROR(ETIMEDOUT);
        }
    }
}
...
int bb_neterrno(void)
{
    ...
    case WSAEINTR:
        return BVERROR(EINTR);
    case WSAEPROTONOSUPPORT:
        return BVERROR(EPROTONOSUPPORT);
    case WSAETIMEDOUT:
        return BVERROR(ETIMEDOUT);
    case WSAECONNREFUSED:
        return BVERROR(ECONNREFUSED);
    case WSAEINPROGRESS:
        return BVERROR(EINPROGRESS);
    }
    return -err;
}
#endif
                   ...
                   socklen_t addrlen, int timeout, BVURLContext *h)
{
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse))) {
        bv_log(NULL, BV_LOG_WARNING, "setsockopt(SO_REUSEADDR) failed\n");
    }
